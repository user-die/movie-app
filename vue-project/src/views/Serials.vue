...
async function getRandomFilms() {
  let response = await axios(
    `https://api.kinopoisk.dev/v1.4/movie?page=1&limit=20&isSeries=true`,
    options['request1']
  )

  let randomPage = randomInteger(1, response.data.pages)

  getFilms(`movie?page=${randomPage}&limit=20&isSeries=false`, randomFilms)
}

onMounted(() => {
  /*
  getFilms2('/collections?type=TOP_POPULAR_MOVIES&page=1', popularFilms)

  getFilms('movie?page=1&limit=20&sortField=votes.kp&sortType=-1&isSeries=false', mostViewed)
  ...
  getFilms2(`/premieres?year=${year}&month=${month.toUpperCase()}&page=1`, latestFilms)

  getFilms(`movie?page=${randomInteger(1, 25)}&limit=20&lists=top250`, bestsFilms)
